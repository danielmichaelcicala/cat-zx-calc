...
\author[1]{Daniel Cicala}
\affil[1]{University of California, Riverside, 900 University Ave, Riverside, CA 92521, USA\\
  \texttt{cicala@math.ucr.edu}}
\authorrunning{D.~Cicala} %mandatory. First: Use abbreviated first/middle names. Second (only in severe cases): Use first author plus 'et. al.'

\Copyright{Daniel Cicala}%mandatory, please use full first names. LIPIcs license is "CC-BY";  http://creativecommons.org/licenses/by/3.0/

\subjclass{F.4.2 Grammars and Other Rewriting Systems}% mandatory: Please choose ACM 1998 classifications from http://www.acm.org/about/class/ccs98-html . E.g., cite as "F.1.1 Models of Computation". 
\keywords{category theory, rewriting, quantum computing, categorical quantum mechanics, categorification, higher category theory}% mandatory: Please provide 1-5 keywords
...
\begin{figure}
	\fbox{%
		\begin{minipage}{1\textwidth}
			...
			\quad \quad
			\subcaptionbox{Red spider}[2 cm]{%
				\centering
				\includegraphics{generater_red_spider}
			}
			\quad \quad
			\subcaptionbox{Hadamard}[2 cm]{%
				\centering
...
Recall that there are two ways to compose  $2$-cells in a bicatgory.  Horizontal composition in $\mathbf{Sp}(\mathbf{Csp}(\mathbf{C}))$ uses pushouts and vertical composition uses pullbacks, as illustrated here:
\begin{equation}
\label{eq:Hor and Vert Composition}
	\begin{minipage}{0.45\textwidth}
		\includegraphics{scC_horizontal_composition} 
	\end{minipage}
	\quad
	\begin{minipage}{0.45\textwidth}
		\includegraphics{scC_vertical_composition}
	\end{minipage}
\end{equation}
...
The concept of $\mathbf{Rewrite}$ is that the $1$-cells are open graphs whose inputs are outputs are chosen by the $0$-cells, and the $2$-cells are rewrite rules that preserves the input and output nodes.  By rewrite rules, we mean those taken from the double pushout graph rewriting approach \cite{Corradini_AlgAppGraphTrans}. 
...
\begin{example}
	...
	\[
		\begin{minipage}{0.33\textwidth}
			\centering
			\includegraphics{wire_over_Szx}
		\end{minipage}		
%			\quad \quad \quad \quad
		\begin{minipage}{0.34\textwidth}
			\centering
			\includegraphics{Hadamard_over_Szx}
		\end{minipage}
%			\quad \quad \quad \quad
		\begin{minipage}{0.33\textwidth}
			\centering
			\includegraphics{Diamond_over_Szx}
		\end{minipage}
	\]
	\[
		\begin{minipage}{0.5\textwidth}
			\centering
			\includegraphics{green_spider_over_Szx}
		\end{minipage}
%			\quad \quad \quad \quad
		\begin{minipage}{0.5\textwidth}
			\centering
			\includegraphics{red_spider_over_Szx}
		\end{minipage}
	\]
...
\begin{figure}[t]
	\fbox{%
		\begin{minipage}{\textwidth}
			...
			\linebreak
			%%%%%%%%%%%%%%%%%%%%%%
			\subcaptionbox{Bialgebra}{%
				\centering
				\includegraphics[scale=0.9]{2cell_bialgebra}
			}
			\quad \quad \quad
			\subcaptionbox{Copy}{%
				\centering
				\includegraphics[scale=0.9]{2cell_copy}
			}
			\linebreak
			%%%%%%%%%%%%%%%%%%%%%%
			\subcaptionbox{$\pi$-copy}{%
				\centering
				\includegraphics[scale=0.9]{2cell_pi_copy}
			}
			\quad \quad \quad
			\subcaptionbox{Cup}{%
				\centering
				\includegraphics[scale=0.9]{2cell_cup}
			}
			\linebreak
			%%%%%%%%%%%%%%%%%%%%%%
			\subcaptionbox{Trivial spider}{%
				\centering
				\includegraphics[scale=1]{2cell_trivial_spider}
			}
			\quad \quad
			\subcaptionbox{$\pi$-commutation}{%
				\centering
				\includegraphics[scale=1]{2cell_pi_commutation}
			}
			\linebreak
			%%%%%%%%%%%%%%%%%%%%%%
			\subcaptionbox{Color change}{%
				\centering
