...
\begin{ex}
	\label{ex:basic_graph_over_Szx}
	%
	Let $S_{\text{zx}}$ be the graph
	%
	\begin{equation}
	\label{diag:zx_struture_graph}
	...
	\[
	\label{fig:ZX_1cells_generators}
...
\begin{ex}
	\label{ex:open_graph_over_Szx}
	Consider the graph over $S_{\text{zx}}$ 
	in Example \ref{ex:graph_over_Szx}.  
...
depicted in Figure \ref{fig:ZX_equations}.  
We claim that the
